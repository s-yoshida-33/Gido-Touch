...
*   **接続先**:
    *   REST API: `http://localhost:[PORT]/api/shops`
...
*   **更新方式**: SSE を使用したプッシュ通知により、店舗データの変更をリアルタイムに検知して反映します。
...
本バージョンより、店舗情報の取得においてポーリング（定期問い合わせ）を廃止し、SSE (Server-Sent Events) を用いたプッシュ通知型への移行を行いました。
これにより、ネットワーク負荷の低減とリアルタイム性の向上を実現しています。

### BridgeWebPopper (店舗情報)
*   **データ取得フロー (キャッシュ戦略)**:
    1.  **初回起動時**: API (`/api/shops`) から全店舗データを取得し、メモリ上にキャッシュします。
    2.  **通常表示時**: 画面遷移や再描画時は**メモリキャッシュ**を使用します。これにより、API通信を発生させずに高速にレンダリングを行います。
    3.  **データ更新時**: BridgeGround サーバーからの SSE 通知 (`update` イベント) を受信すると、自動的に API から最新データを再取得 (`forceReload`) し、キャッシュを更新して画面に反映させます。
*   **SSE イベント仕様**:
    *   `connected`: 接続確立時。念のため最新データを再取得します。
    *   `update`: 店舗データの追加・変更・削除時。最新データを再取得します。
    *   `heartbeat`: 接続維持用（30秒間隔）。
*   **リクエスト重複排除**:
    *   複数のコンポーネント（マップ、リストなど）が同時にデータ要求を行っても、APIリクエストは1回にまとめられます。
*   **エラー時の再接続**:
    *   SSE 接続が切断された場合は、3秒後に自動再接続を試みます。
